...
#
# Images: anything in this repo's screenshots/ directory (generated by
# `python3 generate_screenshots.py` -- see that file's own docstring) gets
...
if [ -d "$SCREENSHOTS_DIR" ] && [ -n "$(ls -A "$SCREENSHOTS_DIR" 2>/dev/null)" ]; then
    echo "==> Copying screenshots into the wiki's images/ folder..."
    mkdir -p "${WORK_DIR}/images"
    IMAGE_COUNT=0
    for img in "$SCREENSHOTS_DIR"/*.png "$SCREENSHOTS_DIR"/*.jpg "$SCREENSHOTS_DIR"/*.jpeg; do
        [ -f "$img" ] || continue
        cp "$img" "${WORK_DIR}/images/"
        echo "images/$(basename "$img")" >> "${WORK_DIR}/.wiki-sync-manifest"
        IMAGE_COUNT=$((IMAGE_COUNT + 1))
    done
    if [ "$IMAGE_COUNT" -gt 0 ]; then
        # Re-sort/dedupe now that the image copy loop appended more lines
        # onto the manifest Python already wrote -- same file, same pruning
        # logic below, just combined from two sources.
        sort -u "${WORK_DIR}/.wiki-sync-manifest" -o "${WORK_DIR}/.wiki-sync-manifest"
        echo "==> Copied ${IMAGE_COUNT} image(s)."
    fi
else
    echo "==> No screenshots/ directory (or it's empty) -- run 'python3 generate_screenshots.py' first if you want images. Skipping."
fi
